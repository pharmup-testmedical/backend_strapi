...
const parseReceiptData = async (qrLink, { strapi }) => {
    try {
        let apiUrl = qrLink;
        ...
        strapi.log.info(
            '[Receipt] First 200 chars of response:',
        ...
        const taxAmount = taxes.reduce((sum, tax) => sum + (tax.sum || 0), 0);
        ...
        // Extract products
        const products = ticket.items.map((item, index) => {
            ...
            const productData = {
                ...
                measureUnit: commodity.measureUnitCode ? data.measureUnits?.[commodity.measureUnitCode] || 'unit' : 'unit',
