...
module.exports = async () => {
  ...
  // НАЙДЕНО НА ПРОДЕ 2026-09-17: изначально при !==1 миграция бросала
  // исключение — а миграции в этом плагине фатальны для старта Strapi
  // целиком (crash-loop, весь сайт лежит, не только эта фича). В проде
  // оказалось 2 РЕАЛЬНЫХ поставщика (не 0) — генуинная неоднозначность,
  // которую и раньше, и сейчас нельзя разрешать угадыванием. Но сама
  // неоднозначность в ОДНОРАЗОВОМ бэкфилле — не повод ронять всё
  // приложение: она уже подразумевает ручной путь («Атрибуцию нужно
  // проставить вручную через Product.cashbackSupplier на каждом товаре» —
  // это было написано в тексте ошибки с самого начала), значит блокировать
  // старт незачем. И 0, и >1 теперь — warn + skip (миграция помечается
  // применённой), а не throw. Без атрибуции существующие товары/позиции
  // просто продолжают работать как раньше (без привязки к депозиту — уже
  // отдельно предусмотренное и проверенное поведение, не ошибка), пока
  // админ не расставит cashbackSupplier вручную там, где это нужно.
  if (suppliers.length !== 1) {
    const names = suppliers.map((s) => `${s.name} (id=${s.id})`).join(', ');
    strapi.log.warn(
      `⚠️ Бэкфилл атрибуции пропущен: ожидался ровно 1 поставщик в базе, найдено ${suppliers.length}` +
        (names ? ` [${names}]` : '') +
        '. Атрибуцию нужно проставить вручную через Product.cashbackSupplier на каждом товаре — приложение запускается штатно, без бэкфилла.'
    );
    return;
  }
